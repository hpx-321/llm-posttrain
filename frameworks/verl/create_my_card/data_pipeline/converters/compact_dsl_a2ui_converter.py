...
_DYNAMIC_NUMBER_PROPERTIES = frozenset({"layoutWeight"})
...
def _validate_component_property_types(
    ...
    for property_name, value in props.items():
        if property_name in _NUMBER_PROPERTIES:
            if (
                property_name in _DYNAMIC_NUMBER_PROPERTIES
                and _is_dynamic_binding(value)
            ):
                continue
...
def _require_literal_or_binding(
    ...
    if literal_type in {int, float} and isinstance(value, bool):
        is_literal = False
    if is_literal or _is_dynamic_binding(value):
        return
    raise CompactDslConversionError(
...
def _require_numeric_or_binding(
    ...
    if isinstance(value, bool):
        is_number = False
    if is_number or _is_dynamic_binding(value):
        return
    raise CompactDslConversionError(
...
def _validate_optional_bool(
    ...
    if isinstance(value, bool) or _is_dynamic_binding(value):
        return
    raise CompactDslConversionError(
...
def _is_expression_binding(value: Any) -> bool:
    return (
        isinstance(value, dict)
        and set(value) == {"expression"}
        and isinstance(value.get("expression"), str)
    )


def _is_dynamic_binding(value: Any) -> bool:
    return _is_path_binding(value) or _is_expression_binding(value)


def _expression_binding_paths(value: dict[str, Any], context: str) -> list[str]:
    if set(value) != {"expression"}:
        raise CompactDslConversionError(
            f"{context}: an expression binding must contain only the expression field."
        )
    expression = value.get("expression")
    if not isinstance(expression, str) or not expression.strip():
        raise CompactDslConversionError(
            f"{context}: expression binding must contain a non-empty string."
        )
    if expression != expression.strip():
        raise CompactDslConversionError(
            f"{context}: expression binding must not have outer whitespace."
        )
    for fragment in ("{{", "}}", "$item", "$__dataModel"):
        if fragment in expression:
            raise CompactDslConversionError(
                f'{context}: expression binding contains forbidden fragment "{fragment}".'
            )
    ...
    while index < len(expression):
        ...
        if expression.startswith("${", index):
            ...
            if end < 0:
                raise CompactDslConversionError(
                    f"{context}: expression binding has an unterminated path."
                )
            path = expression[index + 2 : end]
            if not path.startswith("/") or "{" in path:
                raise CompactDslConversionError(
                    f"{context}: expression binding contains an invalid path."
                )
            _decode_json_pointer(path)
            paths.append(path)
            index = end + 1
        ...
        if character in {"'", '"'}:
            ...
            while index < len(expression):
                ...
                raise CompactDslConversionError(
                    f"{context}: expression binding has an unterminated string."
                )
            continue
        number = re.match(r"(?:\d+(?:\.\d*)?|\.\d+)", expression[index:])
        if number is not None:
            index += len(number.group(0))
            continue
        matched_literal = False
        for literal in ("true", "false", "null"):
            if not expression.startswith(literal, index):
                continue
            end = index + len(literal)
            if end < len(expression) and (
        ...
        operator = next(
            (
                candidate
                for candidate in _EXPRESSION_OPERATORS
        ...
        if operator is not None:
            ...
            continue
        raise CompactDslConversionError(
            f"{context}: expression binding contains unsupported syntax "
            f"at offset {index}."
        )

    if not paths:
        raise CompactDslConversionError(
            f"{context}: expression binding must reference at least one data path."
        )
    if len(paths) == 1 and expression == f"${{{paths[0]}}}":
        raise CompactDslConversionError(
            f"{context}: use a path binding for a single unmodified data path."
        )
    return paths
...
def _validate_source_value(value: Any, context: str) -> None:
    ...
    if "expression" in value:
        _expression_binding_paths(value, context)
        return
...
def _collect_binding_paths(value: Any, paths: list[str]) -> None:
    if isinstance(value, dict):
        if set(value) == {"path"}:
            ...
            return
        if set(value) == {"expression"}:
            paths.extend(_expression_binding_paths(value, "expression binding"))
            return
        for child_value in value.values():
...
def _replace_binding_paths(
    ...
    if isinstance(value, dict):
        ...
        if set(value) == {"expression"} and isinstance(
            value.get("expression"), str
        ):
            expression = _EXPRESSION_PATH_RE.sub(
                lambda match: "${"
                + path_replacements.get(match.group(1), match.group(1))
                + "}",
                value["expression"],
            )
